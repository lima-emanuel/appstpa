...
from Database.performance import DB_Pef_Performance_Resource, DB_Saf_Pef_Requirement, DB_Sec_Pef_Requirement, \
...
from Database.security import DB_Sec_Stride, DB_Sec_Stride_DFD, DB_Sec_Stride_Priority, DB_Sec_Stride_Requirement, \
...
def create_all_tables():
    ...
    if conn is not None:
        ...
        # Security
        create_table(conn, DB_Sec_Stride.create_table())
        DB_Sec_Stride.insert_stride(conn)
        create_table(conn, DB_Sec_Stride_DFD.create_table())
        DB_Sec_Stride_DFD.insert_stride(conn)
        create_table(conn, DB_Sec_Stride_Priority.create_table())
        DB_Sec_Stride_Priority.insert_stride(conn)
        create_table(conn, DB_Sec_Stride_Requirement.create_table())
        ...
        create_table(conn, DB_Sec_Stride_Requirement_Import.create_table())
        create_table(conn, DB_Project_Files.create_table())
        # alter_table(conn, "ALTER TABLE project_files ADD COLUMN order_file INTEGER NOT NULL DEFAULT 1")
        ...
        alter_table(conn, "ALTER TABLE sec_stride_requirement ADD COLUMN id_project REFERENCES project(id);")

        alter_table(conn, "ALTER TABLE saf_loss_scenario_req ADD COLUMN mechanism TEXT DEFAULT ''")
        alter_table(conn, "ALTER TABLE sec_stride_requirement ADD COLUMN mechanism TEXT DEFAULT ''")
        alter_table(conn, "ALTER TABLE sec_stride_requirement ADD COLUMN control TEXT DEFAULT ''")

        #Performance
        create_table(conn, DB_Pef_Performance_Resource.create_table())
        DB_Pef_Performance_Resource.insert_into(conn)
        create_table(conn, DB_Bus_Requirement.create_table())
        create_table(conn, DB_Saf_Pef_Requirement.create_table())
        create_table(conn, DB_Sec_Pef_Requirement.create_table())
        create_table(conn, DB_Bus_Pef_Requirement.create_table())

        alter_table(conn, "ALTER TABLE saf_loss_scenario_req ADD COLUMN performance_req TEXT DEFAULT ''")
        alter_table(conn, "ALTER TABLE sec_stride_requirement ADD COLUMN performance_req TEXT DEFAULT ''")
        alter_table(conn, "ALTER TABLE bus_requirement ADD COLUMN performance_req TEXT DEFAULT ''")
        alter_table(conn, "ALTER TABLE saf_loss_scenario_req ADD COLUMN classification TEXT DEFAULT ''")

        create_table(conn, DB_Pef_Res_Requirement.create_table())
