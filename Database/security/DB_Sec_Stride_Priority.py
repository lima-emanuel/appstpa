...
# make a connection
from Objects.security.Stride_Priority import Stride_Priority
...
def create_table():
    sql_query = "CREATE TABLE IF NOT EXISTS sec_stride_priority (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL);"

    return sql_query

# insert the default data on stride
def insert_stride(conn):
    sql = "INSERT OR REPLACE INTO sec_stride_priority(id, name) VALUES(?, ?)"
...
def select_all():
    ...
    with conn:
        cur = conn.cursor()
        cur.execute("SELECT id, name FROM sec_stride_priority")
