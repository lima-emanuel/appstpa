...
def create_table():
    ...
    #                          "id_comp_dst INTEGER NOT NULL, requirement TEXT NOT NULL, cause TEXT NOT NULL, classification TEXT DEFAULT, " \
    ...
    sql_create_goals_table = "CREATE TABLE IF NOT EXISTS saf_loss_scenario_req (id INTEGER PRIMARY KEY AUTOINCREMENT, id_controller INTEGER NOT NULL, " \
                            "id_uca INTEGER NOT NULL, id_project INTEGER NOT NULL, id_comp_cause INTEGER NOT NULL, id_comp_src INTEGER NOT NULL, " \
                            "id_comp_dst INTEGER NOT NULL, requirement TEXT NOT NULL, cause TEXT NOT NULL, id_priority INTEGER DEFAULT 1 REFERENCES saf_priority(id), " \
                            "mechanism TEXT DEFAULT '', performance_req TEXT DEFAULT '', classification TEXT DEFAULT '', " \
                            "FOREIGN KEY(id_controller) REFERENCES components(id) " \
                            "FOREIGN KEY(id_uca) REFERENCES saf_uca(id) " \
                            "FOREIGN KEY(id_project) REFERENCES projects(id) " \
                            "FOREIGN KEY(id_comp_cause) REFERENCES components(id) " \
...
def select_requirements_by_controller_resource(id_controller, id_resource):
    ...
    with conn:
        ...
        for row in rows:
            ...
            if row_count != None:
                if row_count[0] > 0:
                    result_list.append(Loss_Scenario_Req(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12], row[13], row[14], DB_Saf_Pef_Requirement.select_by_requirement_without_connection(cur, row[0], id_resource)))
